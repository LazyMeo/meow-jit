...
public:
    ...
    [[nodiscard]] inline MeowObject* as_object() const {
        MeowObject* object = nullptr;
        std::visit([&object](auto&& arg) {
            using T = std::decay_t<decltype(arg)>;
            if constexpr (std::is_pointer_v<T> && std::is_base_of_v<MeowObject, std::remove_pointer_t<T>>) {
                object = arg;
            }
        }, data_);
        return object;
    ...
    inline const int64_t* as_if_int() const noexcept { return std::get_if<Int>(&data_); }
    inline const double* as_if_real() const noexcept { return std::get_if<Real>(&data_); }
    inline MeowObject* const* as_if_object() const noexcept {
        static MeowObject* object = nullptr;
        if (is_object()) {
            object = as_object();
            return &object;
        }
    ...
    inline int64_t* as_if_int() noexcept { return std::get_if<Int>(&data_); }
    inline double* as_if_real() noexcept { return std::get_if<Real>(&data_); }
    inline MeowObject** as_if_object() noexcept {
        static MeowObject* object = nullptr;
        if (is_object()) {
            object = as_object();
            return &object;
