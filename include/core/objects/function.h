...
namespace meow::core::objects {
    ...
    public:
        ...
        inline void close(const_reference_type value) noexcept { 
            closed_ = value; 
            state_ = State::CLOSED; 
        }
        inline bool is_closed() const noexcept { return state_ == State::CLOSED; }
        inline const_reference_type get_value() const noexcept { return closed_; }
    ...
    private:
        using chunk_type = meow::runtime::Chunk;
    ...
    public:
        explicit ObjFunctionProto(size_t registers = 0, size_t upvalues = 0, string_type name = nullptr, chunk_type&& chunk) noexcept
        ...
        [[nodiscard]] inline const chunk_type& get_chunk() const noexcept { return chunk_; }
    ...
    private:
        ...
        using upvalue_type = meow::core::Upvalue;
        using visitor_type = meow::memory::GCVisitor;

        proto_type proto_;
        std::vector<upvalue_type> upvalues_;
    public:
        explicit ObjClosure(proto_type proto = nullptr) noexcept: proto_(proto), upvalues_(proto ? proto->get_num_upvalues() : 0) {}

        [[nodiscard]] inline proto_type get_proto() const noexcept { return proto_; }
        /// @brief Unchecked upvalue access. For performance-critical code
        [[nodiscard]] inline upvalue_type get_upvalue(size_t index) const noexcept {
            return upvalues_[index];
        }
        /// @brief Unchecked upvalue modification. For performance-critical code
        [[nodiscard]] inline void set_upvalue(size_t index, upvalue_type upvalue) noexcept {
            upvalues_[index] = upvalue;
        }
        /// @brief Checked upvalue access. Throws if index is OOB
        [[nodiscard]] inline upvalue_type at_upvalue(size_t index) const {
